...
export default class CameraDetail extends Component {
  ...
  */
  getCameraInfo() {
    fetch(
      'http://api.natphoto.me/cameras/' + this.props.match.params.camera_name,
      {
        method: 'GET',
        dataType: 'json'
      }
  ...
  */
  getPhotosForCamera() {
    fetch(
      'http://api.natphoto.me/photos?camera=' +
        this.props.match.params.camera_name,
      {
        method: 'GET',
        dataType: 'json'
      }
  ...
  */
  getParksForCamera() {
    fetch(
      'http://api.natphoto.me/parks?camera=' +
        this.props.match.params.camera_name,
      {
        method: 'GET',
        dataType: 'json'
  ...
  */
  ...
  getCameraInfoMarkup(cameraLabels) {
    if (this.state.amazon_url === '') {
      return (
        <div className="noCamData">
          Sorry, we don't have any data for this camera yet. You can check it
          out
          <a
            href={
              'https://www.amazon.com/s/ref=nb_sb_noss_2?url=search-alias%3Daps&field-keywords=' +
      ...
      return (
        <div>
          <DetailHeader
            pic={this.state.image_url}
            name={this.state.name}
            infoAttributes={cameraLabels}
          />
...
*/
  render() {
    if (this.state.valid === 'unknown') {
      ...
      return (
        <div>
          <h1 className="cameraHeader">
            <span>{this.state.name}</span>
          </h1>
          {this.getCameraInfoMarkup(cameraLabels)}
